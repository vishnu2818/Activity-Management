...
class MailActivity(models.Model):
    ...
    def onchange_supervisor_id(self):
        if self.user_id:
            ...
            if model_id:
                self.res_model_id = model_id.id
                self.res_id = self.user_id.partner_id.id
            ...
            for activity in self:
                ...
                if activity.chaining_type == 'trigger':
                    ...
                    vals.update({
                        ...
                        'res_model_id': self.env['ir.model'].sudo()._get(activity.res_model).id,
                    })
                ...
                record.message_post_with_view(
                    ...
                    attachment_ids=[Command.link(
                        attachment_id) for attachment_id in attachment_ids] if attachment_ids else [],
                )
...
class ResUsers(models.Model):
    ...
    def systray_get_activities(self):
        ...
        model_names = {n[0]: n[1]
                       for n in self.env['ir.model'].sudo().browse(model_ids).name_get()}
...
class ActivityDashboard(models.Model):
    ...
    def get_sh_crm_activity_planned_count_tbl(self, filter_date, filter_user, start_date, end_date, filter_supervisor):
        ...
        planned_doman = expression.AND([doman.copy(), [('state','!=','done'),('date_deadline','>=',fields.Date.today())]])
        ...
        overdue_doman = expression.AND([doman.copy(), [('state','!=','done'),('date_deadline','<',fields.Date.today())]])
